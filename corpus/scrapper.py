...
def write_poem(cleaner_poem, title, author_dir):
    ...
    with open(os.path.join(author_dir, title + '.txt'), 'r+', encoding='utf-8') as poem:
...
if __name__ == "__main__":
    """
        Run `python scrapper.py`. Gets as much poems of an author provided the
        url to its poemhunter page is given. May sometimes fails (ping time out
        for ex.) but the last poem scrapped should be given in the terminal 
        (update your urls.txt file to resume). 
    """
